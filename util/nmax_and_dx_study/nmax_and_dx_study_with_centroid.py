...
Two-zone dx approach
--------------------
Hits are split per shower into two distance zones based on Chebyshev distance
from the origin:  cheb = max(|x|, |y|).

    zone "near"  :  cheb <  --zone-boundary   (default 10 000 m)
    zone "far"   :  cheb >= --zone-boundary

Each zone is clustered and evaluated independently with its own dx sweep:
    --dx-sweep              applied to near-zone hits  (e.g. 5 8 10 15 20)
    --dx-after-10k-sweep    applied to far-zone hits   (e.g. 10 16 20 30 40)

The CSV gains two extra columns:
    zone          – "near" or "far"
    zone_boundary – value of --zone-boundary in metres
...
def _split_by_zone(df_sub, zone_boundary: float, np):
    """
    Split df_sub into two DataFrames based on Chebyshev distance from origin.

    zone "near" : max(|x|, |y|) <  zone_boundary
    zone "far"  : max(|x|, |y|) >= zone_boundary
...
CSV_COLUMNS = [
    ...
    "incident_energy",
    "particle_type",
    "zone",            # "near" (cheb < zone_boundary) or "far" (cheb >= zone_boundary)
    "zone_boundary",   # metres — value of --zone-boundary
    "dx",
    "nmax",
    "total_energy",
    "retained_energy",
...
def process_chunk(
    ...
    incident_pdg: int,
    chunk_id: int,
    dx_sweep_per_particle: dict,        # near-zone dx sweep per particle type
    dx_after_per_particle: dict,        # far-zone dx sweep per particle type
    zone_boundary: float,               # metres; cheb < boundary → "near"
    nmax_sweep: list[int],
    ...
    # Validate / normalise far-zone dx sweeps.
    dx_after_resolved: dict[str, list[float]] = {}
    for pkey in active:
        cfg = PARTICLE_CONFIGS[pkey]
        sweep = dx_after_per_particle.get(pkey) or cfg["default_dx_sweep"]
        sweep = sorted({float(v) for v in sweep})
        if not sweep:
            raise ValueError(f"Empty far-zone dx sweep for particle '{pkey}'.")
        if any(v <= 0 for v in sweep):
            raise ValueError(f"Far-zone dx values must be positive for '{pkey}': {sweep}")
        dx_after_resolved[pkey] = sweep

    if zone_boundary <= 0:
        raise ValueError(f"zone_boundary must be positive, got {zone_boundary}")
    ...
    result = {
        ...
        "n_dedup_removed":  0,
        "particles":        active,
        "dx_sweep_near":    dx_sweep_resolved,
        "dx_sweep_far":     dx_after_resolved,
    ...
    with open(csv_path, "w", newline="") as fcsv:
        ...
        for i, pq_path in enumerate(paths_to_use):
            ...
            for pkey in active:
                ...
                # Split into near / far zones by Chebyshev distance
                df_near, df_far = _split_by_zone(df_sub, zone_boundary, np)

                zones = [
                    ("near", df_near, dx_sweep_resolved[pkey]),
                    ("far",  df_far,  dx_after_resolved[pkey]),
                ]

                for zone_name, df_zone, dx_list in zones:
                    for dx in dx_list:
                        cell_E = _cluster_energies_for_type(
                            df_zone, cell_size=dx, np=np
                        )
                        rows = _energy_ignored_for_nmax_sweep(
                            cell_E, nmax_sweep, np
                        )
                        for r in rows:
                            writer.writerow({
                                "shower_id":           shower_id,
...
def main():
    ...
    parser.add_argument("--dx-sweep", type=float, nargs="+", default=None,
                        help="Shortcut: apply this near-zone dx sweep to e/μ/γ "
                             "(overrides the per-particle --*-dx-sweep flags).")

    # Far-zone dx sweep (beyond --zone-boundary)
    parser.add_argument("--dx-after-10k-sweep", type=float, nargs="+",
                        default=None,
                        help="dx values applied to hits with cheb >= --zone-boundary "
                             "(default: same as --dx-sweep). "
                             "Example: --dx-after-10k-sweep 10 16 20 30 40")
    parser.add_argument("--zone-boundary", type=float, default=10_000.0,
                        help="Chebyshev distance threshold in metres separating "
                             "near and far zones (default: 10000 m = 10 km).")
    ...
    # Far-zone dx sweep: use --dx-after-10k-sweep if provided,
    # otherwise fall back to the near-zone sweep.
    after_base = args.dx_after_10k_sweep or (args.dx_sweep or None)
    if after_base:
        dx_after_per_particle = {
            "electrons": list(after_base),
            "muons":     list(after_base),
            "photons":   list(after_base),
        }
    else:
        dx_after_per_particle = dict(dx_sweep_per_particle)
    ...
    print(f"Near dx (< {args.zone_boundary:.0f}m): "
          f"e={dx_sweep_per_particle['electrons']} "
          f"μ={dx_sweep_per_particle['muons']} "
          f"γ={dx_sweep_per_particle['photons']}")
    print(f"Far  dx (>={args.zone_boundary:.0f}m): "
          f"e={dx_after_per_particle['electrons']} "
          f"μ={dx_after_per_particle['muons']} "
          f"γ={dx_after_per_particle['photons']}")
    ...
    result = process_chunk(
        ...
        dx_after_per_particle = dx_after_per_particle,
