...
Usage
-----
python /n/home04/hhanif/TAMBO-opt/nmax_and_dx_study/nmax_and_dx_study_with_centroid_submission.py \
    --registry-dir /n/netscratch/arguelles_delgado_lab/Everyone/hhanif/tambo_simulations_samples_for_training/ \
    --output-dir   /n/home04/hhanif/TAMBO-opt/results/nmax_study_with_centroids  \
    --total-per-pdg 5000 --chunks-per-pdg 50 \
    --dx-sweep           5 8 10 15 20 \
    --dx-after-10k-sweep 10 16 20 30 40 \
    --zone-boundary      10000 \
    --nmax-sweep  4000 6000 10000 14000 18000 20000 25000 \
    --seed 42 \
    --worker-script /n/home04/hhanif/TAMBO-opt/nmax_and_dx_study/nmax_and_dx_study_with_centroid.py


python /n/home04/hhanif/TAMBO-opt/nmax_and_dx_study/nmax_and_dx_study_with_centroid_submission.py \
    --registry-dir /n/netscratch/arguelles_delgado_lab/Everyone/hhanif/tambo_simulations_samples_for_training/ \
    --output-dir   /n/home04/hhanif/TAMBO-opt/results/nmax_study_with_centroids_2  \
    --total-per-pdg 5000 --chunks-per-pdg 50 \
    --dx-sweep           5 8 10 15 20 \
    --dx-after-10k-sweep 10 16 20 30 40 \
    --zone-boundary      10000 \
    --nmax-sweep 20000 22000 24000 26000 28000 30000 \
    --seed 42 \
    --worker-script /n/home04/hhanif/TAMBO-opt/nmax_and_dx_study/nmax_and_dx_study_with_centroid.py
...
python "{worker_script}" \
    ...
    --dx-sweep           {dx_sweep} \
    --dx-after-10k-sweep {dx_after_sweep} \
    --zone-boundary      {zone_boundary} \
...
def main():
    ...
    p.add_argument("--dx-after-10k-sweep", type=float, nargs="+", default=[10, 16, 20, 30, 40],
                   help="dx values for hits beyond --zone-boundary (default: 10 16 20 30 40).")
    p.add_argument("--zone-boundary", type=float, default=10_000.0,
                   help="Chebyshev distance threshold in metres (default: 10000 m).")
    ...
    print(f"Near dx sweep  : {args.dx_sweep}")
    print(f"Far  dx sweep  : {args.dx_after_10k_sweep}  (beyond {args.zone_boundary:.0f} m)")
    ...
    sbatch_text = SBATCH_TEMPLATE.format(
        ...
        dx_after_sweep = " ".join(str(v) for v in args.dx_after_10k_sweep),
